...
> 测试按层组织：
> - 后端 pytest：`backend/tests/`（87 用例，独立测试库 `stayops_test`）
> - 前端单元/组件测试（Vitest）：`frontend/src/**/__tests__/`（74 用例）
> - 端到端测试（Playwright）：`frontend/e2e/`（10 用例，独立 `stayops_test` 库 + 专用端口）
...
4. 单 worker 串行执行 10 条用例；测试结束 webServer 自动停止
