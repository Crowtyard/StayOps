...
def test_seed_idempotent_and_booking_role_mapping(_database):
    ...
    assert {"ai_manager:use", "ai_manager:manage"} <= mapping["SUPER_ADMIN"]
    assert {"ai_manager:use", "ai_manager:manage"} <= mapping["MANAGER"]
    assert "ai_manager:use" in mapping["FRONT_DESK"]
    assert "ai_manager:manage" not in mapping["FRONT_DESK"]
    assert "ai_manager:use" in mapping["FINANCE"]
    assert "ai_manager:manage" not in mapping["FINANCE"]
    assert "ai_manager:use" not in mapping["HOUSEKEEPING"]
    assert "ai_manager:use" not in mapping["MAINTENANCE"]
